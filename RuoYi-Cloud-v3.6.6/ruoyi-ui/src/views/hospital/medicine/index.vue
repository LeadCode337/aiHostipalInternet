      ...
      <el-table-column label="药品类型" align="center" prop="type">
        <template #default="scope">
          <el-tag :type="scope.row.type === '0' ? 'warning' : 'success'" size="small">
            {{ scope.row.type === '0' ? '处方' : '非处方' }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="药品标签" align="center" prop="content" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="药品类型" prop="type">
              <el-select v-model="form.type" placeholder="请选择药品状态">
                <el-option label="处方" value="0" />
                <el-option label="非处方" value="1" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="药品标签" prop="medicineTagId">
              <el-select v-model="form.medicineTagId" placeholder="请选择药品标签" clearable style="width: 100%;">
                <el-option v-for="tag in medicineTagList" :key="tag.id" :label="tag.content" :value="tag.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="库存数量" prop="stock">
              <el-input v-model="form.stock" placeholder="自动赋值" type="number" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
import { listMedicineTag } from "@/api/hospital/medicineTag"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listMedicineTag().then(response => {
        this.medicineTagList = response.rows || []
      })
    },
    // 取消按钮
    cancel() {
